...
public:
	...
	bintree<T>  suma(bintree<T> const & arbol2)  {
		if (arbol2.empty()) return *this;
		if (this->empty()) return arbol2;
		return{ this->left().suma(arbol2.left()), this->root() + arbol2.root(), this->right().suma(arbol2.right()) };
	}
...
bool caso(){
	bintree<int> arbol1 = read(-1);
	bintree<int> arbol2 = read(-1);
	bintree<int> arbol3;

	if (arbol1.empty()) {
		if (arbol2.empty()) return false;
		else arbol3 = arbol2;
	}
	else arbol3 = arbol1.suma(arbol2);

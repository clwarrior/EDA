...
public:
	...
	bintree<T> reves(bintree<T> const & arbol){
		if (arbol.empty()) return bintree<T>();
		return{ reves(arbol.right()), arbol.root_->elem_, reves(arbol.left()) };
	}
...
template<class T> 
bintree<T> read(T el, istream & stream = cin){
	T l = s[i++];
	if (l != el){
		bintree<T> left = read(el, stream);
		bintree<T> right = read(el, stream);
		return bintree<T>(left, l, right);
	}
	else return bintree<T>();
}
...
bool caso(string const & s){
	i = 0;
	bintree<char> arbol1 = read(' ');
	cout << arbol1.reves() << '\n';
	return true;
}

int main(){
	while (getline(cin, s)){ caso(s); }
	return 0;
}
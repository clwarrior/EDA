...
template <class T>
bintree<T> leerArbolPostorden(stack<T> & pila) {
	if (!pila.empty()) {
		T raiz = pila.top();
		pila.pop();
		if (raiz == '+' || raiz == '-' || raiz == '*' || raiz == '/' || raiz == '%') {
			auto iz = leerArbolPostorden(pila);
			auto dr = leerArbolPostorden(pila);
			return{ iz, raiz, dr };		}
		else {
			return{ raiz };
		}
...
void mostrarDIR(bintree<char> arbol) {
	if (!arbol.left().empty())
		mostrarDIR(arbol.left());
	if(!arbol.right().empty())
		mostrarDIR(arbol.right());
	
	cout << arbol.root();
}
	

void resuelveCaso(string x){
	stack<char> pila;
	for (int i = 0;i < x.size();++i) {
		pila.push(x[i]);
	}
	bintree<char> arbol = leerArbolPostorden(pila);
	mostrarDIR(arbol);
	cout << '\n';
}
...
int main() {
	string x;
	while (getline(cin, x)) { resuelveCaso(x); }
	return 0;
}